...
pub fn execute(tool: &ToolUse, state: &mut State) -> ToolResult {
    ...
    if changes.is_empty() {
        return ToolResult {
            tool_use_id: tool.id.clone(),
            content: "No changes provided".to_string(),
            is_error: true,
        };
    }
    ...
    if failure_count == 0 {
        ...
        ToolResult {
            tool_use_id: tool.id.clone(),
            content: format!("Partial success: {}/{} applied. Successes: {}. Failures: {}",
                success_count, total_changes, successes.join("; "), failures.join("; ")),
            is_error: false,
        }
    }
}
