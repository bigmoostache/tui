    ...
    fn tool_definitions(&self) -> Vec<ToolDefinition> {
        vec![ToolDefinition {
            ...
            description: concat!(
                ...
                "2. Reusable templates belong in .context-pilot/shared/typst-templates/ — they are version-controlled and shared across workers.\n",
                "3. Documents should #import templates from the shared folder: #import \"../../.context-pilot/shared/typst-templates/report.typ\": report\n",
                "4. When the user asks to create a PDF, first check if a suitable template exists in .context-pilot/shared/typst-templates/. If not, create one there, then create the document that imports it.\n",
                "5. 'typst watch' tracks the FULL dependency tree automatically — imports, images, bibliography files, toml config, even package files. No manual dependency management needed.\n",
                "6. Use 'typst compile' for one-off compilations. Use 'typst watch' for documents under active development.\n",
                "7. @preview/ packages are downloaded from Typst Universe and cached globally at ~/.cache/typst/packages/.",
            ).to_string(),
            params: vec![
